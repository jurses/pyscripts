...
def removeWholeRow(cell, ws):
    for cellRows in ws.iter_rows(min_col = column_index_from_string("A"), max_col = 35, min_row = cell.row, max_row = cell.row):
        for x in cellRows:
            x.value = None

def concatenateColElements(vcell):
    for v in vcell:
        concatS += str(v.value)
        concatS += ","
...
def removeUntilNextChange():
    ...
    for cellRows in ws.iter_rows(min_col = col, max_col = col, min_row = row_init, max_row = row_end):
        if newValue == cellRows[0].value:
            if allRow.get():
                removeWholeRow(cellRows[0], ws) # para toda esa fila
            else:
                cellRows[0].value = None    # para solamente esa columna
        else:
...
Label(root, text = "Columnas referentes").grid(row = 4)
...
Button(root, text="Salir", command=root.quit).grid(row = 5, column = 2, sticky=W, pady=4)
Button(root, text = "Abrir archivo", command = openFile).grid(row = 5, column = 0, sticky = W, pady = 4)
Checkbutton(root, text = "Aplicar eliminación a toda la fila", variable = allRow, onvalue = TRUE, offvalue = FALSE).grid(row = 4, column = 2, sticky = W, pady =4)
